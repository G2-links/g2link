...
class ChatListViewModel @Inject constructor(
    ...
    fun sendBroadcastAlert(message: String) {
        viewModelScope.launch { meshManager.sendBroadcast(message) }
    }
}
...
class ChatViewModel @Inject constructor(
    ...
    val messages: Flow<List<MessageEntity>> = combine(
        userPrefsManager.deviceIdFlow, _contactDeviceId.filterNotNull()
    ) { myId, contactId -> Pair(myId, contactId) }.flatMapLatest { (myId, contactId) ->
    ...
    fun setContact(deviceId: String) { _contactDeviceId.value = deviceId }

    fun sendMessage(content: String, isEmergency: Boolean = false) {
        val recipientId = _contactDeviceId.value ?: return
        viewModelScope.launch { meshManager.sendMessage(recipientId, content, isEmergency) }
    }

    fun shareLocation(lat: Double, lng: Double, accuracy: Float) {
        viewModelScope.launch { meshManager.shareLocation(lat, lng, accuracy) }
    }

    fun markContactAsPriority(type: ContactType) {
        viewModelScope.launch {
            _contactDeviceId.value?.let { id -> contactDao.updateContactType(id, type) }
        }
    }
}
...
class SettingsViewModel @Inject constructor(
    ...
    fun setBatteryMode(mode: BatteryMode) { viewModelScope.launch { userPrefsManager.setBatteryMode(mode) } }
    fun setNotificationsEnabled(enabled: Boolean) { viewModelScope.launch { userPrefsManager.setNotificationsEnabled(enabled) } }
    fun setLocationShareEnabled(enabled: Boolean) { viewModelScope.launch { userPrefsManager.setLocationShareEnabled(enabled) } }
    fun updateDisplayName(name: String) { viewModelScope.launch { userPrefsManager.setDisplayName(name) } }
}
...
class BroadcastViewModel @Inject constructor(
    ...
    fun sendEmergencyBroadcast(message: String) {
        viewModelScope.launch { meshManager.sendBroadcast(message); _broadcastSent.value = true }
    }
    fun resetSentState() { _broadcastSent.value = false }
}
...
class QrViewModel @Inject constructor(
    ...
    val myDeviceId: Flow<String> = userPrefsManager.deviceIdFlow
    val displayName: Flow<String> = userPrefsManager.displayNameFlow
    val qrContent: Flow<String> = combine(myDeviceId, displayName) { id, name -> "DM:$id:$name" }
            ...
            if (parts.size >= 3 && parts[0] == "DM") {
                ...
                viewModelScope.launch {
                    contactDao.insertOrUpdateContact(
                        com.g2link.connect.data.local.entity.ContactEntity(
                            deviceId = deviceId, displayName = name,
